...
53\% pairwise agreement obtained with an initial domain-agnostic rubric. Labels are treated as
...
information quality problem at scale. While over 500 hours of video are uploaded every minute,
...
LLM ensemble annotation mitigates both failure modes. Language models apply the rubric without
creator recognition or domain preference, producing labels that reflect content structure rather
than annotator identity. This property is particularly valuable for platform-scale deployment,
where quality labels must generalize across creator reputations and viewer demographics.
...
HIGH-signal prompt is labeled HIGH by definition. This eliminates annotation ambiguity and
ensures perfect label reliability for training data, consistent with established synthetic
dataset construction practice.
...
consensus). This directly validates that rubric calibration resolves the near-random annotation
agreement observed in the initial labeling attempt.
...
statistical reliability of reported metrics. Confidence intervals are not reported due to test
set size. This paper positions itself as a feasibility study and framework contribution;
large-scale empirical validation is explicitly scoped to future work.
...
metrics fail to capture. We demonstrated that domain-specific rubric calibration resolves
near-random LLM annotation agreement (53\% $\rightarrow$ 82\% pairwise), and that mixing real
...
\newblock {\em arXiv preprint arXiv:1809.04233}, 2018.
