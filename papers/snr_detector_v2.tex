...
three-model consensus and 82\% average pairwise agreement---a substantial improvement over the
...
majority-class baseline of F1~=~0.000. We show that the combination of domain-specific rubrics, a production labeling ensemble, and
real-data augmentation improves both annotation reliability and classification performance
...
The production labeling setup---combining calibrated domain-specific rubrics with the final
judge ensemble---achieved 82\% average pairwise agreement (74\% full three-model consensus),
compared with 53\% in the initial domain-agnostic setup. Weighted inter-judge agreement---assigning 1.0 for unanimous consensus
...
LLM ensemble annotation is intended to reduce both failure modes. Language model annotation can reduce
...
domain-specific prompt explicitly designed to elicit either HIGH or LOW signal content. The
generation prompts and resulting transcripts are released in the project repository (see
...
GitHub repository, providing a timestamped, publicly auditable record. Labels were not modified
...
leakage between splits. At inference time, the domain label can be supplied by the platform
taxonomy, selected by the user, inferred with a lightweight domain classifier, or omitted
entirely; the no-domain ablation in Section 6.2 shows the method remains functional without
this feature.
...
panel reported in a companion paper~\cite{crucible} converged on two of three domains, with the Career domain
...
without exhaustive manual annotation, a domain-specific rubric calibration procedure that, together with the production labeling
ensemble, contributes to improved LLM ensemble annotation reliability, and a deployable classifier that
...
\newblock In {\em Advances in Neural Information Processing Systems}, volume~36, pages 46595--46623, 2023.
...
\newblock {\em Journal of Risk Model Validation}, 14(4), 2020.
...
\bibitem{crucible}
Bidit Das.
\newblock {CRUCIBLE}: A multi-agent framework for automated annotation rubric calibration via iterative inter-judge disagreement resolution.
\newblock Companion manuscript submitted to arXiv, 2026. Code and calibration logs: \url{https://github.com/biditdas18/rubric-calibration-agent}.
...
Each transcript includes domain, signal level (HIGH/LOW), and generated text. Labels follow
the generation prompt: HIGH prompts are intended to produce HIGH-signal transcripts, though
prompt adherence is not guaranteed (see Section 4.2). The exact generation prompts for each
domain and signal level are defined in \texttt{generate\_synthetic.py} in the repository.
